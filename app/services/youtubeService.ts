...
export class YoutubeService {
            ...
            if (res && res.data.items) {
                ...
                _this.GetUploadPlaylistIdsFromChannelIds(youtube, channelIds).then((playlistRes) => {
                    _this.GetPlaylistItemsFromPlaylistIds(youtube, playlistRes).then((subscriptionPlaylistItems) => {
                        ...
                        resolve(allPlaylistItems.sortByFieldAsc('videoDate').slice(0, 50)); // only 50 most recent
  ...
  private GetPlaylistItemsFromPlaylistIds(youtube: youtube_v3.Youtube,
                                          playlistIds: string[]): Promise<YoutubePlaylistItem[][]> {
