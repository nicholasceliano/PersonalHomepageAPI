    ...
    describe("Currency", function() {
        it("returns no err and a list of stockQuoteData", function(done) {
            ...
            .then(res => {
                ...
                expect(res.body.data[0]).to.have.property("currPriceVal");
                expect(res.body.data[0]).to.have.property("lastPriceVal");
                done();
            });
        });
    });
    describe("Gmail", function() {
        it("returns an error because no oAuth Token", function(done) {
            ...
            .then(res => {
                ...
                expect(res.body.msg).to.equal("The API returned an error: googleAuthUID HttpHeader variable " +
    ...
    describe("Location", function() {
        it("returns an err if no lat & lon query parameters", function(done) {
            ...
            .then(res => {
                ...
                expect(res).to.be.an("object");
                expect(res.body.err).to.equal(true);
                expect(res.body.msg).to.equal("The API returned an error: \"lat\" and \"lng\" query " +
                                                "parameters required.");
                expect(res.body.data).to.an("array").to.have.lengthOf(0);
                done();
            });
        });
        it("returns an err if lat & lon query parameters are not floats", function(done) {
            ...
            .then(res => {
                ...
                expect(res).to.be.an("object");
                expect(res.body.err).to.equal(true);
                expect(res.body.msg).to.equal("The API returned an error: \"lat\" and \"lng\" query parameters " +
        ...
        it("returns an err if lat or lon query parameters are out of number range", function(done) {
            ...
            .then(res => {
                ...
                expect(res).to.be.an("object");
                expect(res.body.err).to.equal(true);
                expect(res.body.msg).to.equal("The API returned an error: \"lat\" must be between -90 and 90 " +
    ...
    describe("Weather", function() {
        ...
        it("returns an err if lat & lon query parameters are not floats", function(done) {
            ...
            .then(res => {
                ...
                expect(res.body.err).to.equal(true);
                expect(res.body.msg).to.equal("The API returned an error: \"lat\" and \"lng\" query " +
                                                "parameters must be floating-point numbers.");
                expect(res.body.data).to.an("array").to.have.lengthOf(0);
                done();
            });
        });
        it("returns an err if lat or lon query parameters are out of number range", function(done) {
            ...
            .then(res => {
                ...
                expect(res).to.be.an("object");
                expect(res.body.err).to.equal(true);
                expect(res.body.msg).to.equal("The API returned an error: \"lat\" must be between -90 " +
                                                "and 90 and \"lng\" must be between -180 and 180.");
                expect(res.body.data).to.an("array").to.have.lengthOf(0);
                done();
            });
        });
        it("returns data if lat or lon query parameters are valid numbers", function(done) {
            ...
            .then(res => {
                ...
                expect(res.body.data.forecast).to.an("Array").to.have.lengthOf.above(0);
                done();
            });
        });
    });
    ...
    describe("Youtube", function() {
        it("returns an error because no oAuth Token", function(done) {
            ...
            .then(res => {
                ...
                expect(res).to.be.an("object");
                expect(res.body.err).to.equal(true);
                expect(res.body.msg).to.equal("The API returned an error: googleAuthUID HttpHeader " +
